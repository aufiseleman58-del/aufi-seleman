...
export default function PostCommentsModal({ isOpen, onClose, postId }: PostCommentsModalProps) {
  ...
  const handleDelete = async (commentId: string) => {
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, 'postComments');
    }
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[120] flex items-end md:items-center justify-center pointer-events-none">
          ...
          <motion.div 
            ...
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            className="bg-white w-full max-w-md rounded-t-[32px] md:rounded-[32px] overflow-hidden shadow-2xl relative z-10 flex flex-col max-h-[80vh] pointer-events-auto"
          >
            <div className="p-4 border-b border-border flex items-center justify-between bg-slate-50/50 backdrop-blur-md">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black tracking-tight">{t('post.comments') || 'Comments'}</h3>
                ...
                comments.map((comment) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded-xl bg-slate-100 border border-border overflow-hidden shrink-0 shadow-sm">
                      {comment.userPhoto ? (
                        <img src={comment.userPhoto} alt="" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <p className="text-[11px] font-black text-text-main">{comment.userName}</p>
